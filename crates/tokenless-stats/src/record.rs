...
pub enum OperationType {
    ...
    CompressResponse,
    /// Command rewriting via RTK (`PreToolUse` hook).
    RewriteCommand,
    /// TOON format encoding (`PostToolUse` hook).
    CompressToon,
}
...
impl OperationType {
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::CompressSchema => "compress-schema",
            Self::CompressResponse => "compress-response",
            Self::RewriteCommand => "rewrite-command",
            Self::CompressToon => "compress-toon",
        }
    }
}
...
impl FromStr for OperationType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "compress-schema" => Ok(Self::CompressSchema),
            "compress-response" => Ok(Self::CompressResponse),
            "rewrite-command" => Ok(Self::RewriteCommand),
            "compress-toon" => Ok(Self::CompressToon),
            other => Err(format!("unknown operation type: {other}")),
        }
    }
}

/// A single statistics record stored in the SQLite database.
...
pub struct StatsRecord {
    ...
    /// Original command output (for rewrite-command comparison).
    pub before_output: Option<String>,
    /// Rewritten command output (for rewrite-command comparison).
...
impl StatsRecord {
    ...
    /// Set the original text content before compression.
    #[must_use]
    ...
    /// Set the compressed text content after compression.
    #[must_use]
    ...
    /// Set both before and after text for diff export.
    #[must_use]
    ...
    /// Set both before and after command output for comparison.
    #[must_use]
    ...
    /// Format a one-line summary for list output.
    #[must_use]
    pub fn format_summary_line(&self) -> String {
        ...
        format!(
            ...
            agent = self.agent_id,
            bc = self.before_chars,
            ac = self.after_chars,
            cs = self.chars_saved(),
            bt = self.before_tokens,
            at = self.after_tokens,
            tp = self.tokens_percent(),
        )
    }
}
